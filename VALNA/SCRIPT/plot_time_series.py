...
def add_obsstat(ax, mean, std):
    cpl = plt.errorbar(0, mean, yerr=std, fmt='*', markeredgecolor='k', markersize=20, color='k', linewidth=5)

    import matplotlib.patches as mpatches
    # specify the location of rectangle using: (left,bottom),width,height
    #< mean
    rect = mpatches.Rectangle((-1, mean-(110*std)), 2,(108*std),
                              fill=True,
                              color="darkred",
                              alpha=0.3)
    ...
    #> mean
    rect = mpatches.Rectangle((-1, mean+(2*std)), 2,(110*std),
                              fill=True,
                              color="darkred",
                              alpha=0.3)
    plt.gca().add_patch(rect)
...
# need to deal with multivar
    ...
    for ivar, cvar in enumerate(args.var):
        ...
        # set title
        if (args.title):
            ax[ivar].set_title(args.title[ivar],fontsize=30)
