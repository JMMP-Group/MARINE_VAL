MODULE eos
  ...
  INTERFACE sigma0
     MODULE PROCEDURE sigma0_1d, sigma0_2d
  END INTERFACE
...
CONTAINS
  ...
  FUNCTION sigma0_1d ( ptem, psal, kpi )
    !!---------------------------------------------------------------------
    !!                  ***  FUNCTION sigma0_1d  ***
    !!
    ...
    !!
    !!----------------------------------------------------------------------
    REAL(KIND=4), DIMENSION(kpi), INTENT(in) :: ptem, psal  ! temperature and salinity
    INTEGER(KIND=4),              INTENT(in) :: kpi         ! dimension of 1D arrays
    REAL(KIND=8), DIMENSION(kpi)             :: sigma0_1d      ! returned value

    INTEGER(KIND=4)                   :: ji
    REAL(KIND=8), DIMENSION (kpi)     :: zws
    ...
    sigma0_1d(:) = 0.d0
    DO ji = 1, kpi
       zws(ji) = SQRT( ABS( psal(ji) ) )
    END DO

    !$OMP PARALLEL DO SCHEDULE(RUNTIME)
    DO ji = 1, kpi

       zt  = ptem (ji)          ! interpolated T
       zs  = psal (ji)          ! interpolated S
       zsr = zws  (ji)          ! square root of interpolated S
       ...
       sigma0_1d(ji) = ( zr4*zs + zr3*zsr + zr2 ) *zs + zr1 - zrau0
    END DO
    !$OMP END PARALLEL DO

  END FUNCTION sigma0_1d

  FUNCTION sigma0_2d ( ptem, psal, kpi, kpj)
    !!---------------------------------------------------------------------
    !!                  ***  FUNCTION sigma0  ***
    !!
    !! ** Purpose : Compute the in situ density (ratio rho/rau0) and the
    ...
    REAL(KIND=8), DIMENSION(kpi,kpj)             :: sigma0_2d   ! returned value
    ...
    DO jj = 1, kpj
       DO ji = 1, kpi
          ...
          ! potential volumic mass (reference to the surface)
          sigma0_2d(ji,jj) = ( zr4*zs + zr3*zsr + zr2 ) *zs + zr1 - zrau0
       END DO
    END DO
    !$OMP END PARALLEL DO

  END FUNCTION sigma0_2d
