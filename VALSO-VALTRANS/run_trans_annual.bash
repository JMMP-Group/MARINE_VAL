...
check_files() {
   ...
   files=( ${DATPATH}/${RUNID}/nemoXsec*_${YEARB}*DenmarkStrait_trpsig.nc )
   files1=( ${DATPATH}/${RUNID}/nemoXsec*_${YEARB}*FaroeBankChannel_trpsig.nc )
   files2=( ${DATPATH}/${RUNID}/Gibraltar*_${YEARB}*.nc )
   files3=( ${DATPATH}/${RUNID}/BabElMandeb*_${YEARB}*.nc )
   files4=( ${DATPATH}/${RUNID}/StraitOfHormuz*_${YEARB}*.nc )
   files5=( ${DATPATH}/${RUNID}/LombokStrait*_${YEARB}*.nc )
   files6=( ${DATPATH}/${RUNID}/OmbaiStrait*_${YEARB}*.nc )
   files7=( ${DATPATH}/${RUNID}/TimorPassage*_${YEARB}*.nc )
   ...
   nf6=${#files6[@]}
   nf7=${#files7[@]}
   numbers=($nf $nf1 $nf2 $nf3 $nf4 $nf5 $nf6 $nf7)
   #numbers=($nf)
   minval=`printf "%d\n" "${numbers[@]}" | sort -rn | tail -1`
   echo "$minval nfiles $nf $nf1 $nf2 $nf3 $nf4 $nf5 $nf6 $nf7"
   #echo $minval
}
#=============================================================================================================================
...
for RUNID in `echo $RUNIDS`; do
   ...
   YEAR=$start
      ...
      for MONTH in `printf "%02d " $LSTM`; do
         ...
         echo "month overflow $MONTH $TAG $runACC $runTRP"
