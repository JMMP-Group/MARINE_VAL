        ...
        DO ji=2,npiglo-1
           DO jj = ijmin-1, ijmax+1
              IF ( zdrft(ji,jj) == 0 .AND.  &    ! not under ice_shelf
                   &    bathy(ji,jj) /= 40 .AND.  &    ! but in the ocean
                   &    MINVAL(isfindex_wk(ji-1:ji+1 , jj-1:jj+1)) == -ifill  .AND. &  ! 
                   &    isfindex_wk(ji,jj) == 0 ) THEN
DRAW A RANDOM NUMBER FOR EACH POINT HERE
              END IF
           END DO
        END DO
SUM and normalised  dta(i,j)=dta(i,j)/SUM(dta)*dclv
  ...
  END DO
  ierr = putvar(ncout, id_varout(3), dclv2d, 1, npiglo, npjglo)
