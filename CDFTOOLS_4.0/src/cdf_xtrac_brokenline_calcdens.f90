PROGRAM cdf_xtract_brokenline
  ...
  INTEGER(KIND=4) :: np_icethick, np_icefra, np_den  !  "
  ...
  REAL(KIND=8), DIMENSION(:,:),   ALLOCATABLE :: sigma0sec
  ...
  LOGICAL  :: lsig     = .FALSE.                    ! flag for calculating sigma0 along section
  ...
  IF ( lsig ) ALLOCATE ( sigma0sec(npsecmax-1,npk) )
...
! urot = vtangent (positive toward the end of the section) and vrot = vnormal (>0 on the right)
           ...
           IF(lsig) sigma0sec(:,jk) = sigma0( tempersec(:,jk), salinesec(:,jk), npsec(jsec)-1  )
!!$           IF(lsig) sigma0sec(:,jk) = 0.0
           ...
           ierr = putvar (ncout(jsec), id_varout(np_sig), real(sigma0sec(:,jk)), jk, npsec(jsec)-1, 1, ktime=jt )
           ...
           IF (lsig  ) ierr = putvar (ncout(jsec), id_varout(np_den     ), real(sigma0sec(:,jk)),           jk, npsec(jsec)-1, 1, ktime=jt )
