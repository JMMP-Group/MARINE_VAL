...
for e, runid in enumerate(args.runid):
    ...
    wrkdir = args.datpath[0] + "/" + runid + "/amoc_rho/"
    time   = "time_counter"
    sigma  = "sigma"
    latit  = 'nav_lat'
    amoc   = "zomsfatl"
    ...
    ref_sigma = ds[sigma]
    ref_lat, ref_rho = xr.broadcast(ref_latit, ref_sigma)
    lev = np.arange(-5.,19.,1.)
    cmap = "RdYlBu_r"
    
    norm = TwoSlopeNorm(vmin=np.nanmin(amocz_avg.values), vcenter=0, vmax=np.nanmax(amocz_avg.values))
    plt1 = ax.contourf(ref_lat.T, ref_rho.T, amocz_avg, lev, norm=norm, cmap=cmap) 
    plt.colorbar(plt1, label='AMOC (Sv)')

    cs = ax.contour(ref_lat.T, ref_rho.T,
    ...
    ax.set_ylabel(r'$\sigma_{2000}$ [$kg \; m^{-3}$]')
    ax.set_xlim(-30,80)
    #ax.set_ylim(29.5,37.5)
