...
def load_argument():
    ...
    parser.add_argument("-ofile", metavar='output file', help="output file", type=str, nargs=1, required=False, default=['masked_tmask.nc'])
...
def filter_largest_cluster(array):
    """
    ...
    for t in range(array.shape[0]):
        for olevel in range(array.shape[0]):
            labeled_array, num_features = ndimage.label(array[t,olevel,:,:]) # array of same shape as input array, where non-zero values are labeled with integers starting from 1, with each integer representing a different cluster. Structure option to allow for diagonal connections.
            # labeled_array, num_features = ndimage.label(array[t,olevel,:,:], structure=ndimage.generate_binary_structure(2, 2))
...
def main():
    ...
    masked_tmask = filter_largest_cluster(masked_tmask) # 1 x 75 x 1206 x 1440 array, 0 for all values outside the largest cluster
    masked_tmask.to_netcdf(f"{DATADIR}/{args.ofile[0]}")
